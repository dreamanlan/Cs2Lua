...
readobject(lua_State *L, lua_State *dL, const void *parent, const char *desc) {
  ...
  int n = 0;
  if(LUA_TTABLE==t){
    n = count_table(L, lua_gettop(L));
  }
  char buffer[256];
	snprintf(buffer, 256, "[%d]", n);
	lua_newtable(dL);
	lua_pushstring(dL,"size");
	lua_pushstring(dL, buffer);
	lua_rawset(dL, -3);
	lua_pushstring(dL,desc);
	lua_rawsetp(dL, -2, parent);
...
static void
gen_str_kv(lua_State *dL, luaL_Buffer *b, const char *str, const char *desc) {
	char tmp[32];
	size_t l = snprintf(tmp, sizeof(tmp), "[%s]: ",str);
...
pdesc(lua_State *L, lua_State *dL, int idx, const char * typename) {
	...
	while (lua_next(dL, idx) != 0) {
		...
		while (lua_next(dL, -2) != 0) {
		  if(lua_isstring(dL, -2)){
		    const char * str = luaL_checkstring(dL,-2);
			  const char * desc = luaL_checkstring(dL,-1);
			  gen_str_kv(dL, &b, str, desc);
		  }
		  else{
